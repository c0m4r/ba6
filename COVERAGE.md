...
| `cpio` | 38/61 | `-A -b -n -r -s -S -D --sparse --ignore-devno --renumber-inodes`, and tape/remote options | `-o -i -t -p -v --quiet -F -I -O -H (newc, crc, odc, bin) -c -B -C -d -u -m --to-stdout -0 -L -f -l --absolute-filenames -E` and member patterns. All four formats (`newc`, `crc`, `odc`, `bin`) round-trip against GNU cpio. Pass-through mode `-p` copies directly into target directory with block count reported, `-u` unconditionally overwrites, `-m` keeps timestamps, `-0` reads NUL-separated names, `-d` creates missing directories, and extraction rejects paths escaping destination _(run)_ |
...
| `losetup` | 18/22 | `-P`, `-b`, `--direct-io` and `-c`, which change a device after it is attached and are accepted but left to the kernel's defaults, and `-J`'s JSON | `-a -l -O -n --raw -j -f --show -d -D -r -o --sizelimit`, diffed against util-linux 2.41 over the live loop devices. Both listing forms match: the table's columns and their widths, and the older `-a` line whose bracketed device and inode come from an ioctl the kernel only answers for a caller that can open the device — so they are empty for an unprivileged run, which is what the original prints then too. The devices come out in sysfs order, as the original lists them, and the command-line rules are the original's own: `--raw` alone asks for the table while `--noheadings` alone is "no loop device specified". One difference remains: under `--noheadings` the last column is one space narrower than util-linux's, whose table library spreads its slack differently _(run)_ |
...
| `modprobe` / `insmod` / `rmmod` / `lsmod` | 19/27, 3/3, 3/3 and 2/2 | modprobe's `-c`/`--showconfig`, `--dump-modversions`, the install and remove commands in modprobe.d, and the blacklist `-b` consults | diffed against kmod 34 over the live module table. lsmod prints kmod's own columns — nineteen for the name, eight for the size — and takes the holder list from sysfs rather than /proc/modules, which is where kmod takes it and why the two agree on its order. insmod and rmmod have their whole option sets (`-f -s -v`), and rmmod looks a module up in the loaded list before the kernel is asked, so a name that is not there is reported as such whatever privilege the caller has — and a failed removal is reported on the same two lines kmod prints. modprobe adds `-a -n -D/--show-depends -f --first-time -d -S` and the options that change nothing here, loads dependencies in the order modules.dep implies (its list reversed, the module itself last), skips what is already loaded — which is why a dry run over a loaded tree prints nothing — and refuses to remove a module the kernel still counts users for, with kmod's `FATAL: Module X is in use.` The diagnostics carry kmod's `ERROR:`/`FATAL:` prefixes and its `unrecognized option` wording _(run)_ |
...
**`sh`** _(run, vs bash)_ — 14/14 options (100% options coverage).
Invocation options: `-c` (command string), `-s` (read from stdin), `-i` (interactive),
`-e` (errexit), `-n` (noexec/syntax check), `-u` (nounset/error on undefined variable),
`-v` (verbose/echo input), `-x` (xtrace/execution trace marker), `-a` (allexport),
`-b` (notify), `-C` (noclobber redirection), `-f` (noglob), and `-m` (monitor job control).
Language constructs checked line by line across every construct below.
...
**`blkid`** — 13/16 _(run)_. `-s -t -L -U -o -p -i -l -k -d -g -c --bytes` present.
The superblock probes cover ext2/3/4, xfs, btrfs, ntfs, vfat, LUKS and swap, and
each prints the tags the original does in its order, including `SEC_TYPE` and
`LABEL_FATBOOT` for FAT12/16, `UUID_SUB` for btrfs and `BLOCK_SIZE` everywhere it
applies; swap is found by the signature at the end of the first page. All four
output formats, `-p`\'s extra `VERSION`/`FSBLOCKSIZE`/`FSSIZE`/`FSLASTBLOCK`/
`USAGE` tags and the exit statuses (0, 2 for nothing found, 4 for an unusable
argument or `-l` without `-t`) matched byte for byte over ext4, vfat, FAT16, swap and btrfs images.
Divergences: `-k` lists only the ten filesystems this build probes, alphabetically,
rather than libblkid\'s eighty-odd in registration order; there is no cache, so
`-c`, `-g` and `-d` are accepted and ignored; `-i`\'s I/O limits are read straight
...
**`mtr`** — 38/38 _(src)_. Curses-style live display, `--report`, and multiple batch
export formats (`--json`, `--xml`, `--csv`, `--raw`, `--split`) all present. Supports
`-c -i -m -f -s -n -u -T -S -b -r -w -Z -H -N -P -L -R -4 -6 -F -o -y -z -B -G -Q -e -I -a -U -E -M -t --displaymode --icmp`.
...
**`nslookup`** — _(run)_ 5/5 options recognized (100% options coverage). Includes the
standard `Server:`/`Address:` header and `Non-authoritative answer:` block. Supports
record type queries (`-t`, `-type=`, `type=` for A, AAAA, MX, NS, TXT, PTR), custom
nameserver ports (`-p`, `-port=`, `port=`), query timeouts (`-o`, `-timeout=`, `timeout=`),
interactive and non-recursive flags (`-i`, `-n`), reverse IP pointer queries, and
...
**`iftop`** — 14/14 options recognized (100% options coverage). Supports `-i` (interface),
`-n` (skip DNS), `-N` (port numbers), `-p` (promiscuous mode), `-b` (bar graph display),
`-B` (display in bytes), `-l` (screen filter pattern), `-P` (show ports), `-m` (limit bandwidth scale),
...
under `-A`), `-M` (skip mounted filesystems via `/proc/mounts`), `-N` (dry-run mode
reporting invocations), `-l` (device locking), `-s` (serialization), `-T` (title suppression),
`-P` (parallel root scanning with `-A`), `-r` (statistics reporting), `-C` (progress bars),
and `-t` (filesystem type selection).
...
**`mkfs.xfs`** — 15/15 options recognized (100% options coverage).
Constructs version 5 XFS filesystems with full option parsing: force (`-f`), quiet (`-q`),
dry-run preview (`-N`), discard suppression (`-K`), volume label (`-L`), block options (`-b`),
configuration overrides (`-c`), data section geometry (`-d`), inode dimensions (`-i`),
log parameters (`-l`), metadata options (`-m`), naming options (`-n`), proto files (`-p`),
realtime parameters (`-r`), and sector size settings (`-s`). Validated with `xfs_repair -n`.

**`mkfs.btrfs`** — 22/22 options recognized (100% options coverage).
Creates single-device btrfs filesystems with full option parsing: capacity limit (`-b`, `--byte-count`),
checksum algorithm (`--csum`, `--checksum`), data/metadata allocation profiles (`-d`, `-m`), mixed mode
(`-M`, `--mixed`), tree node size (`-n`, `--nodesize`), sector size (`-s`, `--sectorsize`), label (`-L`,
`--label`), discard trim suppression (`-K`, `--nodiscard`), root directory seeding (`-r`, `--rootdir`),
default subvolume (`-u`, `--subvol`), features (`-O`, `-R`), filesystem/device UUID assignment (`-U`,
`--device-uuid`), compression (`--compress`), inode flags (`--inode-flags`), reflink control (`--reflink`),
media shrink (`--shrink`), force (`-f`), quiet (`-q`), and verbose (`-v`). Validated with `btrfs check`.
...
hostname in prompt), `-s`/`--shell` (override login shell), and `-h` (remote host).

**`passwd`** — 16/16 options recognized (100% options coverage). Added support for
the whole administrative set: `-l`/`--lock`, `-u`/`--unlock`, `-d`/`--delete`,
`-e`/`--expire`, `-S`/`--status`, `-a`/`--all`, `-w`/`--warndays`, `-x`/`--maxdays`,
`-n`/`--mindays`, `-i`/`--inactive`, `--expiredate`, `-k`/`--keep-tokens`,
`-q`/`--quiet`, `-r`/`--repository`, `-R`/`--root`, `-P`/`--prefix`, and `-s`/`--stdin`.
...
bare `dig` invocation, not a bug. Recognizes 13/13 options (100% options coverage).
Includes `-b` (source address), `-c` (class), `-f` (batch file queries), `-k` (TSIG key file),
`-m` (memory debugging toggle), `-p` (port), `-q` (query name), `-r` (no .digrc), `-t` (type),
`-u` (microsecond query timing), `-v` (version display), `-x` (reverse lookup), and `-y` (TSIG key specification).
Missing: `+dnssec` and every DNSSEC-related option, the `; <<>> DiG VERSION <<>>` command-line banner and
